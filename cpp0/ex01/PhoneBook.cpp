...
void	PhoneBook::search(void)
{
	...
	for (size_t i = 0; i < FIELD_LEN - 2; i++)
		std::cout << std::setw(COL_LEN) << putstr_inrow(g_fields[i]) << (i != FIELD_LEN - 3 ? "|" : "");
	std::cout  << std::endl;
	for (int i = 0; i < CONTACTS_LEN && !this->_contacts[i]._infos[0].empty(); i++)
	{
		std::cout << std::setw(COL_LEN) << i + 1 << "|";
		for (int j = 0; j < FIELD_LEN - 2; j++)
			std::cout << std::setw(COL_LEN) << putstr_inrow(this->_contacts[i]._infos[j]) << (j != FIELD_LEN - 3 ? "|" : "");
		std::cout <<  std::endl;
	}
